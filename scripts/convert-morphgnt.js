...
// ギリシャ語からカタカナ音写を生成
const greekToKatakanaMap = {
  'α': 'ア', 'ά': 'ア', 'ὰ': 'ア', 'ᾶ': 'ア', 'ἀ': 'ア', 'ἁ': 'ハ', 'ἄ': 'ア', 'ἅ': 'ハ',
  'β': 'ブ',
  'γ': 'グ',
  'δ': 'ド',
  'ε': 'エ', 'έ': 'エ', 'ὲ': 'エ', 'ἐ': 'エ', 'ἑ': 'ヘ', 'ἔ': 'エ', 'ἕ': 'ヘ',
  'ζ': 'ズ',
  'η': 'エー', 'ή': 'エー', 'ὴ': 'エー', 'ῆ': 'エー', 'ἠ': 'エー', 'ἡ': 'ヘー',
  'θ': 'ス',
  'ι': 'イ', 'ί': 'イ', 'ὶ': 'イ', 'ῖ': 'イ', 'ἰ': 'イ', 'ἱ': 'ヒ',
  'κ': 'ク',
  'λ': 'ル',
  'μ': 'ム',
  'ν': 'ン',
  ...
  'σ': 'ス', 'ς': 'ス',
  'τ': 'ト',
  'υ': 'ウ', 'ύ': 'ウ', 'ὺ': 'ウ', 'ῦ': 'ウ', 'ὐ': 'ウ', 'ὑ': 'フ',
  'φ': 'フ',
  'χ': 'ク',
  'ψ': 'プス',
  'ω': 'オー', 'ώ': 'オー', 'ὼ': 'オー', 'ῶ': 'オー', 'ὠ': 'オー', 'ὡ': 'ホー',
};
...
function toKatakana(greek) {
  ...
  for (let i = 0; i < lower.length; i++) {
    ...
    // 二重子音や特殊なケース
    if (char === 'γ' && (next === 'γ' || next === 'κ' || next === 'χ' || next === 'ξ')) {
      result += 'ン';
      continue;
    }

    // 母音の組み合わせ
    if ((char === 'α' || char === 'ε' || char === 'η' || char === 'ο' || char === 'ω') && next === 'ι') {
      const diphthongs = { 'α': 'アイ', 'ε': 'エイ', 'η': 'エーイ', 'ο': 'オイ', 'ω': 'オーイ' };
      result += diphthongs[char] || '';
      i++;
      continue;
    }
    if ((char === 'α' || char === 'ε' || char === 'η' || char === 'ο') && next === 'υ') {
      const diphthongs = { 'α': 'アウ', 'ε': 'エウ', 'η': 'エーウ', 'ο': 'オウ' };
      result += diphthongs[char] || '';
      i++;
      continue;
    }
    ...
    result += greekToKatakanaMap[char] || char;
  }
