...
interface ChapterViewerProps {
  ...
  onNavigateToReference?: (bookId: string, chapter: number, verse: number) => void;
}
          ...
          <div className="space-y-2">
            {references.map((ref, index) => (
              <div key={index} className="flex items-start gap-2 text-sm">
                <span className="text-amber-700 font-medium min-w-[3rem]">
                  {ref.verse}節
                </span>
                ...
                <button
                  onClick={() => {
                    if (onNavigateToReference) {
                      const verseNum = typeof ref.sourceVerse === 'string'
                        ? parseInt(ref.sourceVerse.split('-')[0])
                        : ref.sourceVerse;
                      onNavigateToReference(ref.sourceBook, ref.sourceChapter, verseNum);
                    }
                  }}
                ...
                <span className={`text-xs px-1.5 py-0.5 rounded ${
                  ref.type === 'quote' ? 'bg-green-100 text-green-700' :
                  ref.type === 'allusion' ? 'bg-purple-100 text-purple-700' :
                  'bg-gray-100 text-gray-600'
                }`}>
                  {ref.type === 'quote' ? '引用' : ref.type === 'allusion' ? '暗示' : '参照'}
                </span>
                {ref.note && (
                  <span className="text-gray-500 text-xs">- {ref.note}</span>
                )}
              </div>
            ))}
          </div>
